...
function mostrarLote(nombreLote) {
  ...
  for (let i = 0; i < puntos.length - 1; i++) {
    ...
    // Dibujar marcadores con tooltip que muestre nombre y coordenadas
    puntos.forEach((p, i) => {
    const spot = loteFiltrado[i];
    const nombre = nombreCol ? spot[nombreCol] : '';
    const lat = p[0].toFixed(6);
    const lng = p[1].toFixed(6);
    L.circleMarker(p, { radius: 4, color })
      .bindTooltip(`${nombre ? nombre + '<br>' : ''}Lat: ${lat}, Lon: ${lng}`)
      .addTo(capaMapa);
  });
